...
for k = mink:kincrement:maxk
    for p = minp:pincrement:maxp        
        for ss = simul_iter_low:simul_iter_high
            ...
            RBNpdummydummy = boolCellGrid(topology, numCells,numGenes, k, p, ...
                interaction, lastRBNpdummyStates(:,:,end), RBNp.initTtable, RBNp.initvarF, perturb);
            RBNpdummydummy.update_all(delta);
            lastRBNpdummydummyStates = RBNpdummydummy.allStates(:,:,2:end);
            ...
            RBNp.allStates = cat(3, RBNp.allStates, lastRBNpdummydummyStates);
            RBNpstar2_ssDist = ssDist(RBNp);
...
%             
            ...
            for i = 1:numCells
               ...
               P_delta2 = RBNpstar2_ssDist(i, :);
               converge = abs(.5*(KLD(P,P_delta) + KLD(P_delta,P)) - ...
                   .5*(KLD(P_delta,P_delta2) + KLD(P_delta2,P_delta)))/ ...
                   abs(.5*(KLD(P_delta,P_delta2) + KLD(P_delta2,P_delta)))
