classdef boolCellGridPerturb < matlab.mixin.Copyable
    ...
    methods        
        % Constructor
        function obj = boolCellGridPerturb(topology,numCells,numGenes,k,p,bandwidth, initState, initTruth, initVar, perturb)
            ...
            for cellPos=1:numCells
                allCells{cellPos}=boolCellPerturb(numGenes,k,p,bandwidth,Ttable,varF,outCells,inCells,perturb);
        ...
        function plot_cells(obj, save, delta ,dt)
            ...
            if (save == true)
                ...
                mov(1:3000) = struct('cdata', [], 'colormap', []);
            end      
            ...
            if isLine
                ...
                for jT = 1:delta:3000;%size(obj.allStates,3)
            ...
            for jCell = 1:obj.numCells
                ...
                for jIn = 1:length(thisCell.inCells)
                    ...
                    for jNeigh = 1:size(neigh,2)
                        ...
                        % Dead Cell Perturbation 
                        % Cell 12 causes neighboring Cells 11, 2, 22, 13
                        % to receive '0' input from Cell 12
                        if (jCell == 11 && jNeigh==2)
                            neighStates(jNeigh) = 0;
                        end
                        if (jCell == 13 && jNeigh==1)
                            neighStates(jNeigh) = 0;
                        end
                        if (jCell == 2 && jNeigh==4)
                            %disp(jCell);
                        ...
                        if (jCell == 22 && jNeigh==3)
                            neighStates(jNeigh) = 0;
                        end
                        if (jCell == 12 && jNeigh==2)
                            neighStates(jNeigh) = 0;
                        end
                        if (jCell == 14 && jNeigh==1)
                            neighStates(jNeigh) = 0;
                        end
                        if (jCell == 3 && jNeigh==4)
                            %disp(jCell);
                        ...
                        if (jCell == 23 && jNeigh==3)
                            neighStates(jNeigh) = 0;
                        end
                    end

                    % Apply Linear Threshold of 2 (TEMPORARY) <- Make it
                    % dynanmic
